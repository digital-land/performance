...
DATA_FILES=\
	$(DATA_DIR)adoption.csv\
	$(DATA_DIR)quality.csv\
	$(DOWNLOADED_FILES)

DATABASE=$(DATASET_DIR)performance.sqlite3
...
$(DATABASE): $(DOWNLOADED_FILES) $(DATA_FILES) bin/load-data.py
	@mkdir -p $(DATASET_DIR)
	python3 bin/load-data.py

# All docs are now generated by bin/render.py from the database
$(DOCS_DIR)index.html $(DOCS_DIR)adoption/planx/index.html $(DOCS_DIR)project/open-digital-planning/index.html: $(DATABASE) bin/render.py $(TEMPLATES)
	python3 bin/render.py

# Award page needs SVG maps
$(DOCS_DIR)award/index.html: $(DATABASE) bin/render.py $(TEMPLATES) $(CACHE_DIR)point.svg $(CACHE_DIR)local-planning-authority.svg
	python3 bin/render.py
